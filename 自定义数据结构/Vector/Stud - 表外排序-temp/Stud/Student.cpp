...
std::vector<DATA> CStudent::m_arr;
CStudent::CStudent()
{	
}
...
bool CStudent::Delete()
{
	...
	cin >> nNumb;
	int i = 0;
	vector<DATA>::iterator it = m_arr.begin();
	while (i<m_arr.size()){
...
void CStudent::Print(vector<DATA> &cl)
{
	//Sort();
	int i = 0;
	while (i<cl.size()){
...
bool byNumb(int q, int m){//回调函数，没有this指针，只能用全局变量
	return  CStudent::m_arr[q].nNumb < CStudent::m_arr[m].nNumb;
}

bool byName(int q, int m){//回调函数，没有this指针，只能用全局变量
	return  strcmp(CStudent::m_arr[q].sName, CStudent::m_arr[m].sName) < 0;;
}

bool byMath(int q, int m){//回调函数，没有this指针，只能用全局变量
	return  CStudent::m_arr[q].fMath > CStudent::m_arr[m].fMath;
}


void CStudent::PrintPS(int *ps)
{
	int i = 0;
	while (i<m_arr.size()){
		DATA d = m_arr[ps[i]];
