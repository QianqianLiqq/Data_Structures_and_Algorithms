...
void CStudent::Load()
{
	...
	while (fread(&nType, 1, sizeof(nType), pf) == sizeof(nType)){
		if (nType == 1){
			SScn* p = new SScn;
			p->nType = 1;
			fread(&p->nNumb, 1, sizeof(SScn) - sizeof(nType), pf);//按文件中的顺序读取，第一个参数表示读取之后放到哪里
			pData = p;
		}
		if (nType == 0){
			SArt* p = new SArt;
			p->nType = 0;
			fread(&p->nNumb, 1, sizeof(SArt) - sizeof(nType), pf);
			pData = p;
		}
		if (pData)
			m_list.AddTail(pData);
...
void CStudent::Save()
{
	...
	while (pos){
		DATA* pData = m_list.GetNext(pos);
		if (pData->nType == 1)
			fwrite(pData, 1, sizeof(SScn), pf);	//第一个参数表示将哪个地址的数据写入文件
		if(pData->nType == 0)
			fwrite(pData, 1, sizeof(SArt), pf);		
	}
	fclose(pf);
}
