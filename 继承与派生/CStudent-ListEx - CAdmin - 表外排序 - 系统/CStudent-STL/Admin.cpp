...
#include<algorithm>
using namespace std;
CArray< CAdmin::DATA> CAdmin::m_arr;
CAdmin::CAdmin()
{
}
...
void CAdmin::Sort(BY_FUNC pFunc)
{
	int n = m_arr.GetSize(),i=0;
	int *ps= new int[n];
	while (i < n)
		ps[i] = i++;
	sort(ps, ps + m_arr.GetSize(), pFunc);
	PrintPS(ps);
	delete []ps;
}


void CAdmin::PrintPS(int * ps)
{
	int i = 0, n = m_arr.GetSize();
	while (i < n){
		DATA &d = m_arr[ps[i]];
		cout << d.sName << "\t" << d.sPass << "\t";
		cout << (d.nPrior ? "普通用户" : "管理员" )<< endl;
		++i;
	}
	system("pause");
